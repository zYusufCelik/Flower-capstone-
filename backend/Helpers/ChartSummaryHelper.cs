...
namespace backend.Helpers
{
    public static class ChartSummaryHelper
    {
        public static SummaryInfo GenerateSummary(Chart chart)
        {
            ...
            var steps = chart.Processes.Select(p => new StepDto
            {
                StepName = p.ProcessName,
                Shape = p.Shape ?? "",
                Distance = p.Shape == "arrow" ? p.Distance : null,
                Time = p.Shape != "arrow" ? p.Time : null,
                ValueType = p.ValueAdded == true ? "value added" : "non-value added"
            }).ToList();
